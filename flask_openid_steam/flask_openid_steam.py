...
from urllib.parse import quote as url_quote  # this is the new import

from flask import request, session, redirect, current_app, url_for

# from werkzeug.urls import url_quote # this is the original import
...
class OpenIDResponse(object):
    ...
    def __init__(self, resp, extensions):
        ...
        if self.year_of_birth is None:
            self.year_of_birth = softint(
                lookup.get_uri("http://axschema.org/birthDate/birthYear")
            )
            self.month_of_birth = softint(
                lookup.get_uri("http://axschema.org/birthDate/birthMonth")
            )
...
class OpenID(object):
    ...
    def __init__(
        ...
        if isstring(app):
            ...
            warn(
                DeprecationWarning(
                    "OpenID constructor expects application "
    ...
    def get_next_url(self):
        ...
        """
        return (
            self.check_safe_root(request.values.get("next"))
            or self.check_safe_root(request.referrer)
            or (
                self.fallback_endpoint
                and self.check_safe_root(url_for(self.fallback_endpoint))
            )
            or request.url_root
        )

    def check_safe_root(self, url):
        """Checks if a URL is safe.  If it is, it's returned.  Otherwise
        `None` is returned.  If no safe roots are configured, all URLs
        are safe.  If the URL is inside the same app, it's always safe."""
        if url is None:
            return None
    ...
    def attach_reg_info(self, auth_request, keys, optional_keys):
        ...
        auth_request.addExtension(
            SRegRequest(required=sreg_keys, optional=sreg_optional_keys)
        )
    ...
    def loginhandler(self, f):
        ...
        def decorated(*args, **kwargs):
            ...
            if openid_response.status == SUCCESS:
                ...
                self.signal_error(
                    "OpenID authentication failure. Mesage: %s"
                ...
                self.signal_error(
                    "OpenID authentication weird state: %s"
    ...
    def try_login(
        ...
        return redirect(
            auth_request.redirectURL(
                trust_root, self.get_success_url(), immediate=immediate
            )
        )
